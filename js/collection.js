...
import { ownedParts } from './cards.js';
...
export async function renderCollectionGrid(container, ownedCards, { readOnly = false, from = 'collection' } = {}) {
  container.replaceChildren(...PLANES.map((plane) => {
    ...
    const cards = el('div', { class: 'plane-cards' }, [1, 2, 3, 4].map((part) =>
      createCardElement({ planeIdx: plane.idx, part, owned: owned.includes(part), placeholder: owned.length === 0 })));
    return el('div', { class: 'plane-row' }, [head, cards]);
  }));
}

registerScreen('collection', {
  enter: () => renderCollectionGrid($('#collection-grid'), state.profile.ownedCards, { from: 'collection' }),
});
