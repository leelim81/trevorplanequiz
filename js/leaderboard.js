...
registerScreen('leaderboard', {
  enter: async () => {
    ...
    list.replaceChildren(...rows.map((r, i) => el('li', {}, el('button', {
      ...
      el('span', { class: 'lb-name' }, [r.username || '?', el('div', { class: 'lb-sub', text: `${r.totalPoints || 0} pts · ${(r.completedPlanes || []).length}/10 planes · ${(r.badges || []).length} badges` })]),
      el('span', { class: 'lb-score', text: r.bestRun || 0 }),
    ]))));
